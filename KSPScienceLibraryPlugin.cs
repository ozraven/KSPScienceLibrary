...
namespace KSPScienceLibrary
{
    ...
    public sealed class KSPScienceLibraryPlugin : MonoBehaviour
    {
        ...
        public void Start()
        {
            ...
            System.Version ver = Assembly.GetAssembly(typeof(KSPScienceLibraryPlugin)).GetName().Version;
            version = string.Format("Version {0}.{1}   Build {2}.{3}", ver.Major, ver.Minor, ver.Build, ver.Revision);
            windowTitle = string.Format("Science Library {0}", version);
        ...
        public void Update()
        {
            ...
            if(HighLogic.LoadedScene == GameScenes.EDITOR && drawWindow)
            {
                ...
                if(windowPosition.Contains(mousePos))
                    EditorLogic.fetch.Lock(true, true, true, "KSPScienceLibrary");
                else
                    EditorLogic.fetch.Unlock("KSPScienceLibrary");
            }
        }
        ...
        private void OnDraw()
        {
            ...
            if(drawWindow)
            {
                windowPosition = GUI.Window(2345, windowPosition, OnWindow, windowTitle, windowStyle);
                        ...
                        if(available)
                        {
                            ...
                            if(shouldHaveBiome)
                            {
                                foreach(string biome in ResearchAndDevelopment.GetBiomeTags(body))
                                {
                                    ...
                                    {
                                        bool foundBiome = subjectslist.Any(subject => subject.id.Contains(string.Concat("@", body.name)) && subject.id.Contains(biome.Replace(" ", "")));
                                        if(!foundBiome)
                                            continue;
                                    }
